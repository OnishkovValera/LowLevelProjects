...
static struct region alloc_region(void const *addr, size_t query) {
    ...
    block_init(mapped, (block_size) {region_size}, NULL);
    return (struct region) {mapped, region_size, extends};
}
...
static bool split_if_too_big(struct block_header *block, size_t query) {
    ...
    block_init(block2_addr, (block_size) {block->capacity.bytes - block1_capacity.bytes}, block->next);
    block2_addr -> is_free = block_is_free;
...
static struct block_search_result find_good_or_last(struct block_header *restrict block, size_t sz) {
    ...
    struct block_header *res = NULL;

    while (cur) {
        while (cur -> next && try_merge_with_next(cur));
    ...
    return (struct block_search_result) {.type = BSR_REACHED_END_NOT_FOUND, .block = res};
}
...
static struct block_header *grow_heap(struct block_header *restrict last, size_t query) {
    ...
    if (!try_merge_with_next(last)) last = last -> next;
    return last;
}
